...
test('proposta real passa schema, integridade da fonte e validação jurídica existente', async () => { const real = canonicalData.decisions.find(record => record.value.id === 'tjce-0201664-84-2022-8-06-0029'); const proposal = { ...proposalFrom(), payload:structuredClone(real.value) }; const withoutReal = { ...canonicalData, decisions:canonicalData.decisions.filter(record => record !== real) }; assert.deepEqual(await validateCanonicalProposal({ proposal, proposalSchema, decisionSchema, canonicalData:withoutReal, root }), { valid:true,counts:{ decisions:10,theses:1,foundations:6 } }); });
...
test('sandbox valida/builda cópia e rollback preserva acervo oficial', async () => {
  const before = await digestDirectory(new URL('../../data/jurisprudencia/', import.meta.url)); const result = await simulatePromotionSandbox({ repoRoot:root, proposals:[proposalFrom()] }); assert.deepEqual(result, { validated:true,built:true,applied:1,proof_build_was_isolated:true }); assert.equal(await digestDirectory(new URL('../../data/jurisprudencia/', import.meta.url)), before); assert.deepEqual((await readdir(new URL('../../ingestion/sandboxes/', import.meta.url))).filter(n => n !== '.gitkeep'), []);
});
test('rollback também limpa falha induzida e item inválido não é aplicado', async () => { await assert.rejects(simulatePromotionSandbox({ repoRoot:root, proposals:[proposalFrom()], induceFailure:true }), /induced/); assert.deepEqual((await readdir(new URL('../../ingestion/sandboxes/', import.meta.url))).filter(n => n !== '.gitkeep'), []); });
