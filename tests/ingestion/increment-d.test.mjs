...
test('proposta válida passa schema e validação jurídica existente', async () => { const proposal = proposalFrom(); assert.deepEqual(await validateCanonicalProposal({ proposal, proposalSchema, decisionSchema, canonicalData, root }), { valid:true,counts:{ decisions:canonicalData.decisions.length + 1,theses:canonicalData.theses.length,foundations:canonicalData.foundations.length } }); });
