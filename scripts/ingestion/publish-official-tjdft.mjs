...
export function recordToDecision(record, { consultedAt = today() } = {}) {
  ...
  const summary = (ementa || fullText).slice(0, 900).trim();
  return {
    id,
...
export async function collectAndPublish({ terms = DEFAULT_TERMS, pageSize = 25, maxPages = 1, fetchImpl = globalThis.fetch, dryRun = false } = {}) {
  ...
  for (const term of terms) for (let pagina = 0; pagina < maxPages; pagina++) {
    ...
    for (const record of result.records) { const decision = recordToDecision(record); if (decision && !existing.has(decision.id)) records.set(decision.id, { record, decision }); }
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const result = await collectAndPublish({ dryRun: process.argv.includes('--dry-run'), pageSize: Number(process.env.TJDFT_PAGE_SIZE || 25), maxPages: Number(process.env.TJDFT_MAX_PAGES || 1) });
